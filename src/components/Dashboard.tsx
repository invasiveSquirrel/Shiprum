      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <section className="lg:col-span-7 bg-surface-container-high p-8">
          <div className="flex justify-between items-center mb-10">
            ...
            <span className="font-label text-[9px] uppercase text-outline tracking-widest">Active Archives</span>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8">
            {Object.keys(appStats || {}).map((folder) => (
              <div key={folder} className="flex items-center justify-between py-2 border-b border-outline-variant/10 group cursor-pointer">
                <div className="flex items-center gap-4">
                  ...
                  <span className="font-body text-xl capitalize">{folder}</span>
                </div>
                <ChevronRight className="text-outline" size={16} />
              </div>
            ))}
            {Object.keys(appStats || {}).length === 0 && (
              <p className="text-xs text-outline italic">No active directories mapped.</p>
            )}
          </div>
        </section>
        ...
        <section className="lg:col-span-5 bg-surface-container-low p-8 border border-outline-variant/5">
          ...
          <div className="space-y-4">
            {discussions.length > 0 ? (
              discussions.slice(0, 3).map((log) => (
                <div key={log.fullPath} className="bg-surface p-5 hover:bg-surface-container-highest transition-colors cursor-pointer border-l-2 border-primary-container">
                  <div className="flex justify-between items-start mb-2">
                    <h3 className="font-headline text-lg truncate pr-4">{log.title}</h3>
              ...
              <div className="p-8 text-center bg-surface/50 border border-dashed border-outline-variant/20">
                <p className="text-xs text-outline italic">Your discussion history will appear here once initiated.</p>
              </div>
            )}
          </div>
        </section>
